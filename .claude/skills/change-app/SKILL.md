...
- 既存コードの責務を逸脱する変更は禁止
- 新機能を追加する場合は既存の `src/` 構成（機能ごとに独立した `.ts` ファイル、`src/extension.ts` でアクティベーション・登録、`src/types.ts` に型定義、`src/configurations.ts` に設定読み込みロジック）に従う
- 新しい設定項目を追加する場合は `package.json` の `contributes.configuration` にも追加する
